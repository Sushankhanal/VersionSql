/****** Object:  Procedure [dbo].[mobile_get_user_info_with_pin]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[mobile_get_user_info_with_pin]
...
BEGIN
	...
	BEGIN TRY  
		...
		IF(SELECT COUNT(*) FROM tblCard WHERE Card_SerialNo = @card_sn AND tblCard_pin = @pin) > 0
		BEGIN
		...
		BEGIN
			...
			WHERE (tblCard.Card_SerialNo = @card_sn) and Card_Status = 0
		END
		ELSE
		BEGIN
			SELECT      
