/****** Object:  Procedure [dbo].[Wapi_rptPlayerPointRedemptioneReportv2]    Committed by VersionSQL https://www.versionsql.com ******/
...
CREATE PROCEDURE [dbo].[Wapi_rptPlayerPointRedemptioneReportv2]
...
BEGIN
	...
	BEGIN
		...
		inner join tblCardPointTrans CT (NOLOCK) on CT.CardPointTrans_Code = H.Payment_ReceiptNo+ '-3'
		...
		AND (CASE WHEN @strCardType = '' THEN 1 ELSE CASE WHEN C.Card_Type IN (select NAME from dbo.fnSplitstringToTable(@strCardType)) THEN 1 ELSE 0 END END) > 0
		...
		from tblPayment_Header H (NOLOCK) 
		inner join tblCard C (NOLOCK) on H.Payment_UserCode = C.Card_SerialNo
		inner join tblCardPointTrans CT (NOLOCK) on CT.CardPointTrans_Code+ '-3' = H.Payment_ReceiptNo
		inner join tblPayment_Details D (NOLOCK) on D.PaymentD_ReceiptNo = H.Payment_ReceiptNo
		inner join tblCardOwner CO (NOLOCK) on CO.CardOwner_ID = C.Card_CardOwner_ID
		inner join tblProduct P (NOLOCK) on P.Product_ItemCode = D.PaymentD_ItemCode
		where H.Payment_CreatedDate between @strDateFrom and @strDateTo
