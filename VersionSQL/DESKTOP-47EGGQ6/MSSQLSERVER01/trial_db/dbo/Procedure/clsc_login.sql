/****** Object:  Procedure [dbo].[clsc_login]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[clsc_login] @card_sn         NVARCHAR(50), 
                                   @start           DATETIME, 
                                   @end             DATETIME, 
...
begin
	...
	@CardOwnerId        bigint,
	@CardId             bigint
EXEC	@return_value= [dbo].[Local_clsc_login] @card_sn, @start, @end, @app_version, @setting_version,@ip,@imei,@cardOwner_name,@companyCode,
			 @CardOwnerId output,
    @CardId output;
...
EXEC [dbo].[Sync_CardInsertUpdate] @CardId = @CardId;
	...
	begin
	 ...
	 EXEC [dbo].[Local_clsc_login]  @card_sn, @start, @end, @app_version, @setting_version,@ip,@imei,@cardOwner_name,@companyCode
	end
