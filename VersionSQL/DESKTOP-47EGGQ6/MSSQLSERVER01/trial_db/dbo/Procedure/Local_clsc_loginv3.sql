/****** Object:  Procedure [dbo].[Local_clsc_loginv3]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[Local_clsc_loginv3] @card_sn               NVARCHAR(50), 
                                     @start                 DATETIME, 
                                     @end                   DATETIME, 
...
AS
    BEGIN
        ...
        BEGIN TRY
            ...
            DECLARE @PlayerLevel_ID BIGINT= 1;
            SELECT @PlayerLevel_ID = PlayerLevel_ID
            FROM tblPlayerLevel
            WHERE PlayerLevel_Name = @player_level_text
                  AND PlayerLevel_Status = 0;
            ...
            (
				...
				DECLARE @idgg bigint = 0
				SELECT @idgg = c.Card_CardOwner_ID FROM tblCard c with (nolock) WHERE C.Card_SerialNo = @card_sn;

                    UPDATE A
                      SET 
                          CardOwner_Name = @cardOwner_name, 
                          CardOwner_Level = @PlayerLevel_ID, 
                          CardOwner_clsc_allow_play = 0, 
                          CardOwner_clsc_suffix = @CardOwner_clsc_suffix, 
                          CardOwner_clsc_allow_play_message = NULL
                    FROM tblCardOwner A
                    WHERE CardOwner_ID = @idgg;
                    UPDATE C
                      SET 
                          C.Card_Level = @PlayerLevel_ID
                    FROM tblCard C
            ...
            FROM tblCardOwner
				 ...
				 INNER JOIN tblCardType ON tblCard.Card_Type = tblCardType.CardType_ID
				 INNER JOIN tblPlayerLevel ON tblPlayerLevel.PlayerLevel_ID = CardOwner_Level
            WHERE(tblCard.Card_SerialNo = @card_sn)
                 AND Card_Status = 0;
            COMMIT TRANSACTION;
            SET NOCOUNT OFF;
