/****** Object:  Procedure [dbo].[api_GetPendingOrderKitchenPrint]    Committed by VersionSQL https://www.versionsql.com ******/

--exec api_GetSoupCategory('')
--exec api_GetFoodCategory(''
--exec api_GetPendingOrderKitchenPrintv2 'comp02'
CREATE PROCEDURE [dbo].[api_GetPendingOrderKitchenPrint] (
...
BEGIN
	...
	insert into tblPayment_Details_Print_Backup
	select * from tblPayment_Details_Print
	where PaymentD_ID in (select * from @tblTemp_0) 

	delete from tblPayment_Details_Print
	where PaymentD_ID in (select * from @tblTemp_0) 
