/****** Object:  Procedure [dbo].[sp_tblPayment_Header_Add_Normal]    Committed by VersionSQL https://www.versionsql.com ******/
...
CREATE PROCEDURE [dbo].[sp_tblPayment_Header_Add_Normal]
...
BEGIN
	...
	select @strClientUserCode = mUser_Code from tblUserMobile where mUser_Code = 'Cash'
	if @strClientUserCode = ''
	BEGIN
		INSERT INTO tblUserMobile
		(mUser_Code, mUser_LoginID, mUser_Password, mUser_FullName, mUser_MobileNo, 
		mUser_TotalBalance, mUser_RegisterDate, mUser_Status, 
		mUser_CompanyCode, mUser_Remarks, mUser_TotalPoint, mUser_randomCode)
		VALUES     ('Cash','Cash','','Cash','',
		0,GETDATE(),1,@strCompanyCode,'',0,'')
	END
	...
	--- Check Item got mix the product sign or not
		...
		Payment_IsSettlement, Payment_TotalTaxAmt, Payment_CreatedBy, Payment_Remarks, Payment_RedeemAmt, Payment_RedeemPoint, Payment_DiscountMode,
		Payment_CollectionCode, Payment_TableCode,Payment_OrderNumber)
		VALUES (
		...
		@strClientUserCode,@dblGlobalDiscountRate,@dblGlobalDiscountAmt,0,0,@dblTaxAmt, @strPOSUserCode, @strRemarks, 0, 0, @intDiscountMode,
		@strCollectCode,@strTableCode,@strOrderNumber)
