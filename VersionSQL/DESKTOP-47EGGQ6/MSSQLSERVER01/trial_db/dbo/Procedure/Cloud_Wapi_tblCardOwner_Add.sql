/****** Object:  Procedure [dbo].[Cloud_Wapi_tblCardOwner_Add]    Committed by VersionSQL https://www.versionsql.com ******/
...
create PROCEDURE [dbo].[Cloud_Wapi_tblCardOwner_Add]
...
BEGIN
	...
	Declare @intResultCode int = 0
		...
		DECLARE @ID INT
		SELECT @intCount = ISNULL(COUNT(CardOwner_ID), 0) FROM tblCardOwnerCloud NOLOCK 
		...
		if @strCardNumber <> ''
		BEGIN
			SELECT @intCount = ISNULL(COUNT(Card_ID), 0) FROM tblCardCloud NOLOCK 
		...
		BEGIN
			...
			if @dtDOB <> ''
			BEGIN
				update tblCardOwnerCloud set CardOwner_DOB = @dtDOB where CardOwner_ID = @intID
			END
			if @strCardNumber <> ''
			BEGIN
				update tblCardCloud set Card_CardOwner_ID = @intID where Card_SerialNo = @strCardNumber and Card_Status = 0
				select @strCardPin = tblCard_pin from tblCardCloud where Card_SerialNo = @strCardNumber
			END
			if @strPic <> ''
			BEGIN
				update tblCardOwnerCloud set CardOwner_Pic = @strPic where CardOwner_ID = @intID
			END
			...
			BEGIN
				...
				While (Select COUNT(c.Card_ID) from tblCardCloud c (NOLOCK) where c.Card_SerialNo =  @strCardNumber) > 0
				BEGIN
					SELECT  @strCardNumber = cast((9000000000* Rand() + 1000000000) as bigint )
				END

				Declare @intCardType int = 0
				select top 1 @strCompanyCode = Company_Code from tblCompanyCloud c (NOLOCK) Order by Company_ID

				set @strMobileNumber = ''
				INSERT INTO tblCardCloud
