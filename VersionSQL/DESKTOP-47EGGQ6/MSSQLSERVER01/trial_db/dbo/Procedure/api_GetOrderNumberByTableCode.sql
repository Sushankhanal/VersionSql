/****** Object:  Procedure [dbo].[api_GetOrderNumberByTableCode]    Committed by VersionSQL https://www.versionsql.com ******/

--exec api_GetSoupCategory('')
--exec api_GetFoodCategory(''
-- exec api_GetTableList '','comp02'
CREATE PROCEDURE [dbo].[api_GetOrderNumberByTableCode] (
...
BEGIN
	...
	(CASE Table_isCalling WHEN 1 THEN 3 ELSE Table_Status END) as Table_Status ,
	...
	FROM tblTable t inner join tblTableStatus ts on t.Table_Status = ts.TableStatus_ID
	WHERE (Table_CompanyCode = @strCompanyCode)
	AND Table_Code = @strTableCode
