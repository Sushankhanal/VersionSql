/****** Object:  Procedure [dbo].[sp_tblPayment_Refund]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_tblPayment_Refund]
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			WalletTrans_CampaignCode, WalletTrans_PaymentMode, @strQRCodeText
			...
			--Declare @dblCurrentRedeemPointCharge money = 0
			--Declare @dblCurrentRedeemAmtCharge money = 0
			--select @dblCurrentRedeemPointCharge = Payment_RedeemPoint,
			--@dblCurrentRedeemAmtCharge = Payment_RedeemAmt from tblPayment_Header where Payment_ReceiptNo = @strReceiptNo

			--if @dblCurrentRedeemPointCharge <> 0
			--BEGIN
			--	Declare @intRedeemType int
			--	declare @strClientPointRedeemRate money = 0
			--	set @intRedeemType = 3
			--	set @strClientPointRedeemRate = 0
			--	select @strClientPointRedeemRate = WTransType_DefaultPointRate 
			--	from tblWalletTransType  where WTransType_Name ='Void'

			--	Declare @strPointRemarks nvarchar(50) =''
			--	set @strPointRemarks = 'Get back redeemed point, void from ' + @strReceiptNo
			...
			--	VALUES (
			--	@strNewReceiptNo,@strClientUserCode,@intRedeemType,@dblCurrentRedeemPointCharge,@dblCurrentRedeemPointCharge,
			--	@strReceiptNo,@strPointRemarks,@strPOSUserCode,GETDATE(),'',
			--	@strQRCodeText,@dblCurrentRedeemAmtCharge  * -1,@strClientPointRedeemRate)

			--	Update tblUserMobile set mUser_TotalPoint = mUser_TotalPoint + @dblCurrentRedeemPointCharge
			...
			--END
                ...
                '', @strQRCodeText, PointTrans_WalletAmt, PointTrans_Ratio * -1
