/****** Object:  Procedure [dbo].[Local_api_CardMoneyOut]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[Local_api_CardMoneyOut]
...
AS
    BEGIN
        ...
        IF @intType = 40
            BEGIN
                SELECT @dblAmount = Card_Balance
                FROM tblCard
                WHERE Card_SerialNo = @strCardNo;
            END;
        IF @intType = 42
            BEGIN
                SELECT @dblAmount = Card_Balance_Cashable_No
                FROM tblCard
                WHERE Card_SerialNo = @strCardNo;
            END;
        IF @intType = 41
            BEGIN
                SELECT @dblAmount = Card_Balance_Cashable
                FROM tblCard
                WHERE Card_SerialNo = @strCardNo;
            END;
        IF @dblAmount <= 0
